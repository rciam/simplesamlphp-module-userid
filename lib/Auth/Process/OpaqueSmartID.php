    ...
    public function process(&$request) {
        ...
        if (!empty($this->idpTagBlacklist)) {
            if (!empty(array_intersect($this->idpTagBlacklist, $idpTags))) {
                ...
                SimpleSAML_Logger::debug("[OpaqueSmartID] Skipping IdP with tags " . var_export($idpTags, true) . " - blacklisted");
                return;
            }
        }
        ...
        if (!empty($this->idpTagWhitelist)) {
            if (empty(array_intersect($this->idpTagWhitelist, $idpTags))) {
                ...
                SimpleSAML_Logger::debug("[OpaqueSmartID] Skipping IdP with tags " . var_export($idpTags, true) . " - not it whitelist");
                return;
            }
        }
